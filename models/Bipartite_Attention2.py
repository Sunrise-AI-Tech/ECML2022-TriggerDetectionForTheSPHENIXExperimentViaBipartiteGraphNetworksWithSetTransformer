...
class Bipartite_Attention2(nn.Module):
    def __init__(
            self,
            num_features,
        ...
        super(Bipartite_Attention2, self).__init__()
        _layers = []
        ...
        self._pred_layers = nn.Sequential(
                nn.Linear(2*total_feature_dim, prev_dim),
                nn.ReLU(),
    ...
    def forward(self, X):
        ...
        for layer in self._layers:
            ...
            pooled_Xs.append(mean_pooled)
            pooled_Xs.append(max_pooled)
        H = torch.cat(pooled_Xs, axis=-1)
