...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/cluster_of_tracks_pnsp.yaml')
...
def extract_hyperparameters(config):
    ...
    hp['type/physics_model'] = config['physics_model']['type']
    hp['n_features/physics_model'] = config['physics_model']['n_features']
    hp['n_hidden/physics_model'] = config['physics_model']['n_hidden']
    hp['hidden_size/physics_model'] = config['physics_model']['hidden_size']
    hp['hidden_activation/physics_model'] = config['physics_model']['hidden_activation']
...
def train(data, model_pnl, model_physics, model, optimizer, epoch, output_dir, use_wandb=False,  threshold = 0.5, contrast=False, ce_weight=1, contrast_loss_fn=None, use_extra_pos=True):
    train_info = do_epoch(data, model_pnl, model_physics, model, epoch, optimizer, threshold = threshold, contrast=contrast, ce_weight=ce_weight, contrast_loss_fn=contrast_loss_fn, use_extra_pos=use_extra_pos)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model_pnl, model_physics, model, epoch, loss_config=None, threshold=0.5):
    with torch.no_grad():
        val_info = do_epoch(data, model_pnl, model_physics, model, epoch, optimizer=None, threshold=threshold)
    return val_info


def do_epoch(data, model_pnl, model_physics, model, epoch, optimizer=None, threshold = 0.5, contrast=False, ce_weight=1, contrast_loss_fn=None, use_extra_pos=True):
    global writer
    ...
    for batch in data:
        tracks, vtx, partitions_as_graph, n_tracks, trig, energy, momentum, is_trigger_track = batch
        ...
        physics_in = torch.cat((tracks, pred_x), dim=-1)

        physics = model_physics(physics_in)

        data_x = torch.cat((tracks, pred_x, physics), dim=-1)
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    if mconfig['type'] == 'MLP':
        ...
        del mconfig['type']
        model_physics = HitsLSTM(
                **mconfig
            )
    else:
        raise NotImplementedError('Model {config["physics_model"]["type"]} not implemented.')
    ...
    model_pnl.eval()
    checkpoint_file_physics = config['checkpoint_file_physics']
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        train_info = train(train_data, model_pnl, model_physics, model, optimizer, epoch, config['output_dir'], threshold=config['threshold'], contrast=contrast, contrast_loss_fn=contrast_loss_fn, ce_weight=ce_weight,
                )
        table = make_table(
        ...
        val_info = evaluate(val_data, model_pnl, model_physics, model, epoch, threshold=config['threshold'],
                )
        table = make_table(
