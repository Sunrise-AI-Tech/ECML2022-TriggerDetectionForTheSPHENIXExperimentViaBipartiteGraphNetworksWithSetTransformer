...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/gt_track_biatt.yaml')
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    name = config['name_on_wandb'] + f"-mod{config['model_mod']}-n_hid{config['model']['layers_spec'][0][0]}-agg_activation{config['model']['aggregator_activation']}-use_radius{config['data']['use_radius']}-use_momentum{config['data']['use_momentum']}-ntrain{config['data']['n_train']}-lr{config['optimizer']['learning_rate']}-{config['optimizer']['type']}-b{config['data']['batch_size']}"
    logging.info(name)
    if args.use_wandb:
        wandb.init(
