...
def parse_args():
    ...
    argparser.add_argument('--config', default='configs/cluster_of_tracks_pns_corrupt.yaml')
...
def extract_hyperparameters(config):
    ...
    hp['corruption_level/data'] = config['data']['corruption_level']
...
def train(data, model, optimizer, epoch, output_dir, use_wandb=False,  threshold = 0.5, contrast=False, ce_weight=1, contrast_loss_fn=None, use_extra_pos=True, use_energy=False, use_momentum=False):
    train_info = do_epoch(data, model, epoch, optimizer, threshold = threshold, contrast=contrast, ce_weight=ce_weight, contrast_loss_fn=contrast_loss_fn, use_extra_pos=use_extra_pos, use_energy=use_energy, use_momentum=use_momentum)
    write_checkpoint(checkpoint_id=epoch, model=model, optimizer=optimizer, output_dir=output_dir)
    return train_info

def evaluate(data, model, epoch, loss_config=None, threshold=0.5, use_momentum=False, use_energy=False):
    with torch.no_grad():
        val_info = do_epoch(data, model, epoch, optimizer=None, threshold=threshold, use_energy=use_energy, use_momentum=use_momentum)
    return val_info


def do_epoch(data, model, epoch, optimizer=None, threshold = 0.5, contrast=False, ce_weight=1, contrast_loss_fn=None, use_extra_pos=True, use_energy=False, use_momentum=False):
    global writer
    ...
    for batch in data:
        tracks, vtx, A, n_tracks, trig, is_trigger_track, energy, momentum = batch
        ...
        data_edge_batch, data_edge_start, data_edge_end = (A > 0).nonzero(as_tuple=False).t()
        ...
        data_edge_attr = ((A + A.transpose(1, 2)) / 2)[A.to(torch.bool)].flatten().unsqueeze(-1)
...
def main(auto=False, parser_dict=None, trails_number=None, datasets=None):
    ...
    mconfig['num_features'] += 3*config['data']['use_energy'] + config['data']['use_momentum']
    model = SAGPoolNet(
        **mconfig
    )
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        train_info = train(train_data, model, optimizer, epoch, config['output_dir'], threshold=config['threshold'], contrast=contrast, contrast_loss_fn=contrast_loss_fn, ce_weight=ce_weight,
        ...
        val_info = evaluate(val_data, model, epoch, threshold=config['threshold'],
                use_energy=config['data']['use_energy'], use_momentum=config['data']['use_momentum']
                )
        table = make_table(
