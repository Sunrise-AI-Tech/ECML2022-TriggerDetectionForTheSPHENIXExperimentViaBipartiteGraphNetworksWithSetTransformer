...
def load_graph_gt(filename, load_compelete_graph=False):
...
def load_graph_pred(filename, load_compelete_graph=False):
    with np.load(filename) as f:
...
class TrkDataset():
    ...
    def __init__(self, input_dir=None, filelist=None, n_samples=None,
                n_input_dir=1, input_dir2=None, input_dir3=None, input_dir4=None, random_permutation=True, n_samples2=None,
                n_samples3=None, n_samples4=None, load_complete_graph=False, corruption_level=0.0, use_radius=False, first_file_index=0):
        if filelist is not None:
            ...
            if n_input_dir >= 2:
                ...
                n_pred = len(self.filenames)
            if n_input_dir >= 3:
                input_dir3 = os.path.expandvars(input_dir3)
                filenames = sorted([os.path.join(input_dir3, f) for f in os.listdir(input_dir3)
                                if f.startswith('event') and not f.endswith('_ID.npz')])
                # random.shuffle(filenames)
                self.filenames += (filenames if n_samples3 is None else filenames[first_file_index:first_file_index+n_samples3])
            if n_input_dir >= 4:
                input_dir4 = os.path.expandvars(input_dir4)
                filenames = sorted([os.path.join(input_dir4, f) for f in os.listdir(input_dir4)
                                if f.startswith('event') and not f.endswith('_ID.npz')])
                # random.shuffle(filenames)
                self.filenames += (filenames if n_samples4 is None else filenames[first_file_index:first_file_index+n_samples4])
        ...
        for file_index in range(len(self.filenames)):
            # removed ptype
            if file_index < n_pred:
                track_vector, complete_flags, origin_vertices, momentums, pids, energy, trigger, ip, adj, radius, is_trigger_track, n_hits, n_tracks = load_graph_pred(self.filenames[file_index], load_complete_graph)
...
def get_datasets(n_train, n_valid, input_dir=None, filelist=None,
                n_folders=1, input_dir2=None, input_dir3=None, input_dir4=None, random_permutation=True,
                n_train2=None, n_valid2=None, n_train3=0, n_valid3=0, load_complete_graph=False, corruption_level=0.0, use_radius=False):
    # n_train2 = n_train
    # n_valid2 = n_valid
    train_data = TrkDataset(input_dir=input_dir, filelist=filelist,
                           n_samples=n_train, n_samples2=n_train, n_samples3=n_train2, n_samples4=n_train2,
                           n_input_dir=4, input_dir2=input_dir2, input_dir3=input_dir3, input_dir4=input_dir4, 
    ...
    valid_data = TrkDataset(input_dir=input_dir, filelist=filelist,
                           n_samples=n_valid, n_samples2=n_valid, 
                           n_input_dir=2, input_dir2=input_dir2, 
                           random_permutation=random_permutation,
                           load_complete_graph=load_complete_graph,
                           corruption_level=corruption_level, use_radius=use_radius, first_file_index=n_train)
